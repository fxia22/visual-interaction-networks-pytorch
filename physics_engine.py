...
def show_data(xy, resolution=(400,400)):
    ...
    for i in range(xy.shape[0]):
        ...
        for j in range(xy.shape[1]):
            #frame[int(xy[i,j,1]), int(xy[i,j,0]), :] = colors[j]
            mask = ((xx - xy[i,j,1]) ** 2 +  (yy - xy[i,j,0]) ** 2) < 400
            #print(mask.shape)
            frame[mask,:] = colors[j]
...
if __name__ == '__main__':
    ...
    # pkl.dump(data, open('test.pkl', 'wb'))
    # print(pkl.load(open('test.pkl', 'rb')).shape)
    show_data(xy, resolution=(32,32))
    show_data(xy)